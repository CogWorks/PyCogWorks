        ...
        while self.read_messages:
            ...
            if val:
                if val[0] == self.GAZEINFO:
                    ...
                    if self.fp and self.fix_log:
                        self.fix_log.write('%f\t%d\t%f\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\n' % (time.time(), pygame.time.get_ticks(), self.fix_data.timestamp, self.fix_data.gaze_found, self.fix_data.gaze_x, self.fix_data.gaze_y, self.fix_data.eye_motion_state, self.fix_data.fix_x, self.fix_data.fix_y, self.fix_data.gaze_deviation, self.fix_data.sac_duration, self.fix_data.fix_duration))
